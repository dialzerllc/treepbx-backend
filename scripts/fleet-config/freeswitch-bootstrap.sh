...
if [ -d "$PAYLOAD_DIR" ]; then
  ...
  scp $SSH_OPTS "$PAYLOAD_DIR/ai-screen.lua" root@"$FS_IP":/tmp/ai-screen.lua >/dev/null
  ssh $SSH_OPTS root@"$FS_IP" "
    ...
    docker cp /tmp/ai-screen.lua fs:/usr/share/freeswitch/scripts/ai-screen.lua
    rm -f /tmp/voice-broadcast.lua /tmp/02_voice_broadcast.xml /tmp/amd-screen.lua /tmp/ai-screen.lua
  "
fi
